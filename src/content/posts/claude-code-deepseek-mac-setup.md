...
Mac 的終端機在 `應用程式 → 工具程式 → 終端機`，或者直接按 `Cmd + 空白鍵` 搜尋「終端機」。打開後你會看到一個黑底白字的視窗，顯示類似：
...
跑完之後，關掉終端機再重開（`Cmd + Q` 完全退出再打開），或者執行這行讓設定立刻生效：
...
Claude Code 預設用 Anthropic 的官方 API（需要美國手機驗證，對中國用戶不友善）。透過一個代理層，可以讓它串接到 DeepSeek 的 API。
...
Claude Code 支援 `ANTHROPIC_BASE_URL` 環境變數來改變 API 端點。我們需要一個**轉發層**，因為 Claude Code 的通訊格式跟 DeepSeek 不完全相同。可以用 **one-api** 或 **new-api** 這類開源專案，但最簡單的方式是用一個輕量的 Python 腳本。
